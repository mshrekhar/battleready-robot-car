...
void Vive510::processPulse() {
 // static int checkflag=0;
  ...
  if (m_lastFalling != m_usFalling) {
    int pulsewidth = m_usFalling-m_usRising-2;
    ...
    if (pulsewidth > m_sweepWidth) {
      if (pulsewidth > 140) { //pulse too long. bad pulse
        ...
        m_pulseType = KTYPE;
#ifdef DEBUG
  logMessage("\nKPin" + String(m_pin) + " width=" + String(pulsewidth));
#endif
      } 
      else if (isJPulse(pulsewidth)) {
        m_pulseType = JTYPE;
#ifdef DEBUG
  logMessage("\tJPin" + String(m_pin) + " width=" + String(pulsewidth));
#endif
      }
    }
    else { // x sweep or y sweep
#ifdef DEBUG
      logMessage("Pin" + String(m_pin) + ":" + String(pulsewidth) + "  r=" + String(m_usRising-m_lastFalling));
#endif
